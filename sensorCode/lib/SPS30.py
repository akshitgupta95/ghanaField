...
class SPS30:
    ...
    def __init__(
        ...
        self.PM_fields = [
            ...
            ["grain", "mu", self.SPS_TYP_SIZE, None]
            if not explicit
            else ["pm03_cnt", "pcs/0.1dm3", self.SPS_TYP_SIZE, [0, 100]],
        ]
        if type(calibrate) is dict:
            for key in calibrate.keys():
                if calibrate[key] and type(calibrate[key]) != list:
                    continue
    ...
    def receive(self, cmd, debug=False):
        ...
        if len(buf[4:-1]) != buf[3]:
            raise ValueError("length rcv")
        if debug:
            print("Received: %s" % str(buf))
        if ~sum(buf) & 0xFF:
            raise ValueError("checksum")
        if buf[1] != cmd:
            raise ValueError("sensor reply error")
    ...
    def auto_clean(self, interval=None, debug=False):
        if interval == None:
            if debug:
                print("Auto clean, no interval")
            self.send(self.SPS_AUTO_CLEAN, [0x0,], debug=debug)
        else:
            if debug:
                print("Auto clean, interval %d" % interval)
    ...
    def PassiveRead(self, debug=False):
        if self.mode == self.STANDBY:
            if not self.GoPassive(debug=debug):
                return []
    ...
    """
        ...
        if self.explicit:
            ...
            for pmCnt in PM_sample.keys():
                if pmCnt.find("_cnt") < 0:
                    continue
                if pmCnt.find("03_cnt") > 0:
                    PM_sample["pm03_cnt"] = PM10
                else:
                    PM_sample[pmCnt] = PM10 - PM_sample[pmCnt]
        return PM_sample
